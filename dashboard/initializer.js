...
async function processConfigUpdate(type, serverId, config) {
    switch (type) {
        case 'ticket_config':
            await ticketModule.sendAllTicketConfigs(socket); // use module
            break;
        case 'application_config':
            await applicationModule.sendAllApplicationConfigs(socket); // or refresh that specific app
            break;
        case 'autoresponder_config':
            await autoResponderModule.sendAllAutoResponderConfigs(socket); // refresh autoresponder configs
            break;
